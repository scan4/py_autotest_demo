...
- 【正向创建用例的唯一键】创建类成功路径用例（POST 创建用户/物品等），其唯一键字段（如 email）会被数据库唯一约束校验——同一值第二次创建必失败。所以成功创建用例的唯一键必须用随机占位：email 写 "前缀.<random_uuid>@example.com"（执行时自动替换为新随机值），确保每轮执行都能创建成功
